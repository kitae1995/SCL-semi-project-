<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	<script>
        window.onload = function() {
			...
			document.getElementById('moreList').onclick =() =>{
				/*
				왜 false를 주냐? 더보기니까 -> 누적해서 보여줘야하니까 ( 추가하는것처럼 초기화하면 안됨 )
				*/
				getList(++page , false);
			}
            ...
            //게시글 상세보기 화면에 처음 진입했을시 댓글 리스트를 한번 불러오자
            getList(1,true); // 밑에있는 function getList임
            ...
            function getList(pageNum,reset) {
                ...
                .then(data => {
                	...
                	//더보기 버튼 배치 판단
                	//현재페이지번호 * 이번요청으로 받은 댓글 수 > 전체 댓글수 = 더보기 없어도 됨
                	//그렇지만 반대의경우엔 더보기가 있어야함 ( 현재 페이지로는 남은 댓글들을 구현할 수 없으니)
                	...
                	//insertAdjacentHTML : 인접한 곳에 html을 삽입하는 함수 ( 기준위치를 잡아줘야함 )
 ...
 //				getTime() : 1970년 1월 1일 자정을 기준으로 현재까지의 시간을 밀리초로 리턴.
 				...
 				let time;
 				if(gap < 60*60*24*1000){ // 밀리초라 1000곱 해야 초단위가 됨
 					if(gap < 60*60*1000){
 						time = '방금 전';
 					}else{
